...
This NIP specifies use of event kind `2102` with the following structure: 
...
```json
{
  ...
  "tags": [
    ["p", "<wallet_pubkey>"],
    ["e", "<event_id>"] // used only in response by the wallet to ack the request
  ],
...
```json
{
    ...
    "amount_sats": "<must_match_invoice>",
...
```json
{
  ...
  "content": "<NIP-44 encrypted {\"res\":\"ACK\",\"preimage\":\"<lightning_preimage>\"}>",
  "sig": "<signature>"
}
```
...
```json
{ // ...
  "content": "<NIP-44 encrypted {\"res\":\"GFY\",\"code\":\"2.2\",\"message\":\"Unknown Lightning Failure\"}>",
  "sig": "<signature>"
}
```
...
2. **Temporary Failure**: When the wallet service is temporarily unable to process the request. This can include:
   - **Routing Failure**: Unable to find a route to the payment destination.
   - **Unknown Lightning Failure**: An unspecified error occurred in the Lightning Network back-end.
...
The debit link is a bech32-encoded string with the prefix `ndebit`, containing:

- The public key of the listener (user's wallet service)
- The relay URL where the wallet service listens for debit requests
...
5. Source verification: Allow user to inspect the signature of the event.
...
5. Source verification: Inform the user of the signature they can use to verify authenticity.

## Security Considerations

1. All communication should be encrypted using [NIP-44](44.md).
2. Wallet services should implement strong authentication and atomic transactions to ensure only authorized applications can send debit requests and are constrained by budgets.
