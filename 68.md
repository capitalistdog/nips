...
The UX of NWC is flawed in forcing the user to pre-provision keys and define local rules before communicating the URI to a service. This process is not only cumbersome and counter-intuitive to unsophisticated users, but involves key-fumbling of the secret, which introduces intercept vectors. From a services perspective with regards to scaling, NWC posesses suboptimal properties as each client having a unique key necessitates distinct communications processes for each user. Terms of these connections are also not trivially co-established and limits its utility in common scenarios like recurring payments. NWC architecture also fails to leverage the benefits of Nostr becoming a reputation system and undermines wallet services ability to establish spending rules based on an endless variety of graph data. 
...
3. Promote users inspections the event for authenticity and/or provide context known about the requesting key.
...
Applications SHOULD:
1. Provide users with the requesting key and signature they can use to verify authenticity of the request.
